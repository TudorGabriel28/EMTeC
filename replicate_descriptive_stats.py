...
for strategy in ['greedy', 'greedy_search', 'beam', 'beam_search', 'sampling', 'topk', 'topp']:
...
for task in ['summarization', 'synopsis', 'article_synopsis', 'keywords', 'key_words', 'key words', 'words_given']:
    if task in df['task'].values:
        task_data = df[df['task'] == task]
        print(f"\n{task_names.get(task, task)}")
        print("-" * 60)
